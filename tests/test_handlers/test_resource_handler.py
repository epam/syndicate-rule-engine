...
def valid_event():
    ...
    event.customer_name = 'test_customer'
...
def mock_regions():
    ...
    with patch('handlers.resource_handler.get_region_by_cloud_with_global') as mock_get_regions, \
         patch('handlers.resource_handler.AllRegionsWithGlobal') as mock_all_regions:
        mock_get_regions.return_value = ['us-east-1', 'us-west-2', 'global']
        mock_all_regions.__contains__ = lambda self, item: item in ['us-east-1', 'us-west-2', 'eu-west-1', 'global']
...
def test_validate_event_bad_resource_type(resource_handler, invalid_event_bad_resource_type, mock_customer, mock_tenant):
    """Test validation of an event with unsupported resource type."""
    resource_handler._ms.customer_service().get.return_value = mock_customer
    resource_handler._ms.tenant_service().get.return_value = mock_tenant
    resource_handler._rs.get_resource_types_by_cloud.return_value = ['aws.ec2', 'aws.s3']  # Don't include the unsupported type
    
    with pytest.raises(ValueError, match="Resource type aws.unsupported_resource is not supported for cloud AWS"):
        resource_handler._validate_event(invalid_event_bad_resource_type)

def test_validate_event_bad_location(resource_handler, invalid_event_bad_location, mock_customer, mock_tenant, mock_regions):
    ...
    with pytest.raises(ValueError, match="Location invalid-region is not supported for cloud AWS"):
