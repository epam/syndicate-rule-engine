...
```
**Note:** this one by default has `CURRENT` name that must be used to reference this entity.
...
```bash
syndicate re job submit --tenant_name CURRENT --region eu-west-1  # or the region you want
```
...
```bash
syndicate re report resource latest --tenant_name CURRENT --json > data.json
```
...
When an instance is launched from AMI one tenant is created automatically. Its default name is `CURRENT` and it 
...
- configure AWS access keys or access role for that specific tenant:
    ...
    ```bash
    syndicate re job submit_aws --tenant_name CURRENT --access_key $AWS_ACCESS_KEY_ID --secret_key $AWS_SECRET_ACCESS_KEY --session_token $AWS_SESSION_TOKEN --region eu-west-1
    ```
...
```bash
syndicate re license activate --license_key <License key from previous command> --tenant_name CURRENT --tenant_name ANOTHER_ACCOUNT
```
...
To request a scan for `CURRENT` tenant for all regions and all rules you the following command:

```bash
syndicate re job submit --tenant_name CURRENT --region eu-west-1
```
...
```bash
syndicate re job submit --tenant_name CURRENT --ruleset FULL_AWS --region eu-west-1
```
...
```bash
syndicate re job submit --tenant_name CURRENT --ruleset FULL_AWS --rules_to_scan ecc-aws-001... --rules_to_scann ecc-aws-002...
```
...
```bash
syndicate re tenant set_excluded_rules --tenant_name CURRENT --rules ecc-aws-001 --rules ecc-aws-002
```
...
```bash
syndicate re report resource latest --tenant_name CURRENT --format xlsx --region eu-central-1 --name my-lambda --json
```
...
```bash
syndicate re report raw latest --tenant_name CURRENT --obfuscated --json
```
