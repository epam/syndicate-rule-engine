...
- `TENANT_AWS_REGIONS` - aws regions to activate for the tenant that represents this account (default `<all regions>`). Specify regions split by space
- `ADMIN_EMAILS` - customer admin emails split by space (default ``)
- `TENANT_PRIMARY_CONTACTS` - tenant primary emails split by space (default ``)
- `TENANT_SECONDARY_CONTACTS` - tenant secondary emails split by space (default ``)
- `TENANT_MANAGER_CONTACTS` - tenant manager contacts split by space (default ``)
