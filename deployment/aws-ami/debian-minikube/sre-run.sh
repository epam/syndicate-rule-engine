...
export SRE_LOCAL_PATH HELM_RELEASE_NAME GITHUB_REPO FIRST_USER MODULAR_SERVICE_USERNAME RULE_ENGINE_USERNAME THIS_ACCOUNT_TENANT_NAME THIS_ACCOUNT_AWS_REGIONS ADMIN_EMAILS TENANT_PRIMARY_CONTACTS TENANT_SECONDARY_CONTACTS TENANT_MANAGER_CONTACTS TENANT_OWNER_EMAIL
sudo --preserve-env=SRE_LOCAL_PATH,HELM_RELEASE_NAME,GITHUB_REPO,FIRST_USER,MODULAR_SERVICE_USERNAME,RULE_ENGINE_USERNAME,THIS_ACCOUNT_TENANT_NAME,THIS_ACCOUNT_AWS_REGIONS,ADMIN_EMAILS,TENANT_PRIMARY_CONTACTS,TENANT_SECONDARY_CONTACTS,TENANT_MANAGER_CONTACTS,TENANT_OWNER_EMAIL -u "$FIRST_USER" sre-init --system | sudo tee -a "$LOG_PATH" >/dev/null
