...
def azure_custodian_resource_type_map() -> dict[str, str]:
    ...
    for resource_type, class_ in AzureResourceMap.items():
        if hasattr(get_factory(class_).resource_type, 'resource_type'):
            type_map[
                get_factory(class_).resource_type.resource_type.lower()
            ] = resource_type
    return type_map
...
def custodian_azure_resource_type_map() -> dict[str, str]:
    ...
    for resource_type, class_ in AzureResourceMap.items():
        if hasattr(get_factory(class_).resource_type, 'resource_type'):
            type_map[resource_type] = get_factory(
                class_
...
class AzureGraphResourceCollector(BaseResourceCollector):
    ...
    def _load_scan(
        self, data: dict, account_id: str, tenant_name: str, customer_name: str
    ):
        resource_id = data['id']
        ...
        try:
            while True:
                query_options = QueryRequestOptions(
                    skip=skip, top=top, result_format='objectArray'
                )
            ...
            _LOG.error(
                f'Failed to collect Azure resources using Resource Graph: {e}'
            )
            raise
...
class AzureFOCUSResourceCollector(BaseResourceCollector):
    ...
    def _get_latest_focus_blob(self, container_client):
        ...
        if not data_blobs:
            raise ValueError(
                f"No FOCUS export data files found with prefix '{self._blob_prefix}'"
            )
    ...
    def _load_focus_data(self, blob_client, blob_name: str):
        ...
        if blob_name.endswith('.csv.gz'):
            ...
            raise ValueError(
                f'Unsupported file format for blob: {blob_name}. Only CSV and CSV.GZ files are supported.'
            )
    ...
    def _extract_resource_info(self, focus_row: dict) -> dict | None:
        ...
        """
        resource_id = focus_row.get('ResourceId') or focus_row.get(
            'resource_id'
        )
        resource_name = focus_row.get('ResourceName') or focus_row.get(
            'resource_name'
        )
        resource_type = focus_row.get('ResourceType') or focus_row.get(
            'resource_type'
        )
        region = (
            focus_row.get('Region')
            or focus_row.get('region')
            or focus_row.get('AvailabilityZone')
        )
        ...
        if not mapped_resource_type:
            mapped_resource_type = (
                f'azure.{resource_type.lower().replace("/", ".")}'
                if resource_type
                else None
            )
    ...
    def collect_tenant_resources(
        ...
        **kwargs,
        ...
        if tenant.cloud != Cloud.AZURE:
            _LOG.warning(
                f'Tenant {tenant_name} is not Azure tenant. Skipping.'
            )
            return
        ...
        try:
            ...
            _LOG.info(
                f'Saved {resources_saved} unique resources from FOCUS data for tenant {tenant_name}'
            )

        except Exception as e:
            _LOG.error(
                f'Failed to collect FOCUS resources for tenant {tenant_name}: {e}'
            )
            raise
    ...
    def collect_all_resources(
        ...
        **kwargs,
        ...
        for tenant in tenants:
            ...
            try:
                _LOG.info(
                    f'Collecting FOCUS resources for tenant: {tenant.name}'
                )
                self.collect_tenant_resources(
                    tenant_name=tenant.name,
                    regions=regions,
