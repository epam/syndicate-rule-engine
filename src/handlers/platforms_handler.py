...
class PlatformsHandler(AbstractHandler):
    ...
    def post_k8s(
        ...
        application = self.aps.build(
            ...
            meta={PLATFORM_EVENT_DRIVEN_ENABLED_META: False},
        )
        if event.endpoint:
            ...
            secret_name = cl.safe_name(
                name=application.customer_id, prefix='m3.sre.k8s', date=True
            )
    ...
    def get_k8s(self, event: BaseModel, platform_id: str):
        ...
        if not platform:
            raise (
                ...
                .message(self._ps.not_found_message(platform_id))
                .exc()
            )
        self._ps.fetch_application(platform)
        return build_response(content=self._ps.dto(platform))
    ...
    def patch_k8s(self, event: PlatformK8SPutModel, platform_id: str):
        ...
        self._ps.set_event_driven_enabled(
            platform=platform, enabled=event.event_driven_enabled
        )
        self._ps.fetch_application(platform)
        return build_response(content=self._ps.dto(platform))
    ...
    def list_k8s(
        ...
        if event.event_driven_enabled is None:
            ...
            it = self._ps.iter_by_event_driven_enabled(
                enabled=event.event_driven_enabled,
