...
from helpers import build_response, RESPONSE_BAD_REQUEST_CODE, \
    RESPONSE_ACCEPTED_STATUS, RESPONSE_INTERNAL_SERVER_ERROR, \
    RESPONSE_OK_CODE, RESPONSE_RESOURCE_NOT_FOUND_CODE
from helpers.constants import POST_METHOD, ACTION_PARAM_ERROR, \
...
class ConfigurationApiHandler(AbstractApiHandlerLambda):
    ...
    def __init__(self, handlers: List[AbstractHandler],
        ...
        self.REQUEST_PATH_HANDLER_MAPPING = {
            '/rules/update-meta': {
                POST_METHOD: self.invoke_rule_meta_updater
            },
            '/backup': {
                POST_METHOD: self.backup_configuration
            },
            '/metrics/update': {
                POST_METHOD: self.update_metrics
            },
        }
    ...
    def update_metrics(self, event):
        ...
        if response.get('StatusCode') == RESPONSE_ACCEPTED_STATUS:
            _LOG.debug(
        ...
        _LOG.error(
            f'Response code is not {RESPONSE_ACCEPTED_STATUS}. '
...
priority_governance_service = SERVICE_PROVIDER.priority_governance_service()
...
API_HANDLER = ConfigurationApiHandler(
    handlers=[customer_handler, tenant_handler,
              ...
              parent_handler, rabbitmq_handler],
    lambda_client=SERVICE_PROVIDER.lambda_func(),
    rule_source_service=SERVICE_PROVIDER.rule_source_service())
