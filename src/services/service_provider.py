...
class ServiceProvider(metaclass=SingletonMeta):
    ...
    def ruleset_service(self) -> RulesetService:
        ...
        return RulesetService(s3_client=self.s3)
    ...
    @cached_property
    def event_service(self) -> EventStoreService:
        from services.event_driven import EventStoreService

        return EventStoreService(environment_service=self.environment_service)

    @cached_property
    def event_ingest_service(self) -> EventIngestService:
    ...
    def report_statistics_service(self):
        ...
        return ReportStatisticsService(setting_service=self.settings_service)
