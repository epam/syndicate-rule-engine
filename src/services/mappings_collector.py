...
class MappingsCollector:
    ...
    def add_meta(self, metadata: 'Metadata'):
        for name, meta in metadata.rules.items():
            self._severity[name] = meta.severity
            self._mitre[name] = meta.mitre
            self._service_section[name] = meta.service_section
            self._category[name] = meta.category
            self._service[name] = meta.service
            self._standard[name] = meta.standard
            domain = meta.get_domain()
            if domain:
                self._cloud_rules.setdefault(domain.value, []).append(name)
            if meta.cloud:
                self._cloud_rules.setdefault(meta.cloud, []).append(name)
            self._human_data[name] = {
