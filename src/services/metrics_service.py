...
class MetricsService:
    ...
    def custom_modify(
        self, it: ResourcesGenerator, meta: dict
    ) -> ResourcesGenerator:
        """
    ...
    def report_fields(self, rule: str) -> set[str]:
        rf = set(
            self.mappings_collector.human_data.get(rule, {}).get(
                'report_fields'
            )
            or []
        )  # noqa
    ...
    def create_resources_generator(
        self, collection: ShardsCollection, active_regions: set | list
    ) -> ResourcesGenerator:
        ...
        return self.allow_only_regions(
            resources, {GLOBAL_REGION, *active_regions}
        )
...
class CustomerMetricsService(MetricsService):
    ...
    def get_by_customer_date_type(customer, date, _type) -> CustomerMetrics:
        metrics = list(
            CustomerMetrics.customer_date_index.query(
                hash_key=customer,
    ...
    def list_by_date_and_customer(date: str, customer: str, limit: int = 50):
        ...
        while last_evaluated_key:
            try:
                ...
                if (
                    e.cause_response_code
                    == 'ProvisionedThroughputExceededException'
                ):
                    _LOG.warning(
                        'Request rate on CaaSCustomerMetrics table '
                        'is too high!'
                    )
                    time.sleep(10)
                else:
    ...
    @staticmethod
    def get_all_types_by_customer_date(
        customer: str, date: str, overview_only: bool = False
    ):
        result = {
            'ATTACK_VECTOR': False,
            'COMPLIANCE': False,
...
class TenantMetricsService(MetricsService):
        ...
        while last_evaluated_key:
            try:
                ...
                if (
                    ...
                    _LOG.warning(
                        'Request rate on CaaSJobs table is too ' 'high!'
                    )
                    time.sleep(10)
                else:
                    raise e
    ...
    @staticmethod
    def get_by_tenant_date_type(
        tenant: str, date: str, top_type: str
    ) -> TenantMetrics:
        metrics = None
        try:
            metrics = list(
                TenantMetrics.tenant_date_index.query(
                    limit=1,
                    hash_key=tenant,
            ...
            if (
                ...
                _LOG.warning(
                    'Request rate on CaaSCustomerMetrics table ' 'is too high!'
                )
                time.sleep(5)
            else:
                raise e
    ...
    def get_all_types_by_customer_date(date: str, customer: str):
        ...
        for _type, _ in result.items():
            if list(
                TenantMetrics.customer_date_index.query(
                    hash_key=customer,
                    limit=1,
                    range_key_condition=TenantMetrics.date.startswith(date),
                    filter_condition=TenantMetrics.type == _type,
